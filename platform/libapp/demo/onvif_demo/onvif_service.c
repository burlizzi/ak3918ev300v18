...
int GetNet(struct  netinfo_t *resp)
{
	...
	if (ak_net_get_ip("eth0", ip)) {
		ak_print_error_ex("get eth0's ip failed\n");
		return -1;
	}
	if (ak_net_get_netmask("eth0", netmask)) {
		ak_print_error_ex("get eth0's netmask failed\n");
		return -1;
	}
	if (ak_net_get_route("eth0", route)) {
...
int init_network(dvs_t * dvs)
{
	...
	dvs->video_height = 960;//camera_get_ch1_height();
	dvs->video_width_small = 640;
	dvs->video_height_small = 320;//camera_get_ch2_height();
	return 0;
}
